...
public class ReverseDnsIntegrationTest {
    @RegisterExtension
    public static final DockerComposeExtension docker = DockerComposeExtension.builder()
            .files(DockerComposeFiles.from("src/integrationTest/resources/reverse-dns-compose.yaml"))
            .build();
    ...
    public void container_reverse_dns() throws IOException, InterruptedException {
        ...
        assertThat(reverseHostName).matches("db2\\.palantir\\.pt\\..+_default\\.");
        assertThat(reverseHostName2).matches("db\\.palantir\\.pt\\..+_default\\.");
    }
}
