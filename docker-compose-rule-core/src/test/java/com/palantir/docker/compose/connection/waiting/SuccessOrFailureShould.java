...
public class SuccessOrFailureShould {
    ...
    public void not_have_failed_if_actually_a_success() {
        assertThat(SuccessOrFailure.success()).is(new HamcrestCondition<>(is(successful())));
    }

    @Test
    public void have_failed_if_actually_a_failure() {
        assertThat(SuccessOrFailure.failure("oops")).is(new HamcrestCondition<>(is(failure())));
    }

    @Test
    public void return_the_failure_message_if_set() {
        assertThat(SuccessOrFailure.failure("oops")).is(new HamcrestCondition<>(is(failureWithMessage("oops"))));
    }

    @Test
    public void fail_from_an_exception() {
        Exception exception = new RuntimeException("oh no");
        assertThat(SuccessOrFailure.fromException(exception)).is(new HamcrestCondition<>(is(failureWithMessage(both(containsString("RuntimeException")).and(containsString("oh no"))))));
    }

    @Test
    public void succeed_on_a_lambda_that_returns_true() {
        SuccessOrFailure successFromLambda = SuccessOrFailure.onResultOf(() -> true);
        assertThat(successFromLambda).is(new HamcrestCondition<>(is(successful())));
    }
    ...
    public void fail_on_a_lambda_that_throws_an_exception() {
        ...
        assertThat(failureFromLambda).is(new HamcrestCondition<>(is(failureWithMessage(
    ...
    public void fail_on_a_lambda_that_returns_false() {
        ...
        assertThat(failureFromLambda).is(new HamcrestCondition<>(is(failureWithMessage("Attempt to complete healthcheck failed"))));
    }
}
