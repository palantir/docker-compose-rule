...
public class SuccessOrFailureShould {
    ...
    public void fail_from_an_exception() {
        ...
        assertThat(SuccessOrFailure.fromException(exception))
                .is(new HamcrestCondition<>(is(failureWithMessage(
                        both(containsString("RuntimeException")).and(containsString("oh no"))))));
    }
    ...
    public void fail_on_a_lambda_that_returns_false() {
        ...
        assertThat(failureFromLambda)
                .is(new HamcrestCondition<>(is(failureWithMessage("Attempt to complete healthcheck failed"))));
    }
}
